    ...
    <div class="px-8">
        ...
        @forelse ($files as $file)
            <div class="flex gap-4 p-2 rounded-md border-gray-200 border bg-slate-200 mt-4">
                ...
                <div
                    class="text-gray-400 flex-1 flex-col">{{ Carbon::parse($file->created_at)->format('M j, Y') }}</div>
                <div class="text-gray-400 flex-1 flex-col">{{ $file->size }}</div>
                {{--                <div class="text-gray-400 flex-1 flex-col">{{ $file->mime_type }}</div>--}}
            </div>
        @empty
            <div class="colspan-4 flex gap-4 p-2 rounded-md border-gray-200 border bg-red-200 mt-4">No files yet</div>
        @endforelse
    </div>
</div>
