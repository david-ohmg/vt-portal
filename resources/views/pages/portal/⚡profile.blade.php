...
<div>
    ...
    <form wire:submit.prevent="save">
        <div class="flex-col px-8 py-4 bg-slate-100 mx-8 rounded-md border border-gray-200">
            <label class="block text-xs" for="name">Name</label>
            <input class="border border-gray-300 bg-slate-50 rounded p-2 w-full" type="text" id="name" wire:model="name">
            @error('name') <span class="text-red-500">{{ $message }}</span> @enderror
            <label class="block text-xs" for="email">Email</label>
            <input class="border border-gray-300 bg-slate-50 rounded p-2 w-full" type="email" id="email" wire:model="email">
            @error('email') <span class="text-red-500">{{ $message }}</span> @enderror
            <label class="block text-xs" for="password">Password</label>
            <input class="border border-gray-300 bg-slate-50 rounded p-2 w-full" type="password" id="password"
                   wire:model="password">
            @error('password') <span class="text-red-500">{{ $message }}</span> @enderror
            <label class="block text-xs" for="password_confirm">Confirm Password</label>
            <input class="border border-gray-300 bg-slate-50 rounded p-2 w-full" type="password" id="password_confirm"
